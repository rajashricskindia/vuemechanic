<!-- src/views/UsedTractors.vue -->
<template>
  <div class="service">
    <router-link to="/" class="back-icon">
      <i class="fas fa-arrow-left"></i> Back to Home
    </router-link>
    <h1 class="section-title">Used Tractors</h1>
    <h3 class="sub-title">
      Browse our selection of quality used tractors, each carefully inspected for reliability and performance.
    </h3>
    <div class="products">
      <router-link
        v-for="product in products"
        :key="product.id"
        :to="{ name: 'UsedTractorDetails', params: { productId: product.id } }"
        class="product-card"
      >
...
export default {
  ...
  methods: {
    async fetchProducts() {
      try {
        const response = await axios.get(`${API_BASE_URL}/api/products/getall?pstype=2`);
